...
- Custom domains: member centre UI reads/writes the `custom_domains` table (see `migrations/20251114_create_custom_domains.sql`). Point customer subdomains to `edge.lunacirca.com`, then use `/member/custom-domains` to generate the CNAME/TXT instructions.
